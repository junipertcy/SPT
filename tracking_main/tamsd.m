...
for i=j_star
    m=m+1;
    hold on; 
    plot(log([1:50]*0.01),log(msd{i}{1}(1:50)),'-');
    x = log([1:10]*0.01);
    y = log(msd{i}{1}(1:10));
    [p(m,:),ErrorEst] = polyfit(x,y,1);
    D_sigma(m) = (3/(length(traj{i})-2))^0.5;
end
plot(log([1:50]*0.01),log(MSDt),'o-');
plot(log([1:10]*0.01),log(MSDt(1:10)),'o-');

%% fitting MSDt with only the first 10 points
x = log([1:10]*0.01);
y = log(MSDt(1:10));
[pp,ErrorEst] = polyfit(x,y,1);
